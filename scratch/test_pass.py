...
print('=== TESTING REGIONS (Full Record, 0-2000m, Temperature) ===')
for r in regions:
    ...
    s_obs = ssig['observed']
    tc_m = tc['depth_m']
    print(f'Region [{r}]: Floats={f_cnt}, MatchingObs={obs_cnt:,}, TempObs={t_obs}, SalObs={s_obs}, Thermocline={tc_m}')

print('\n=== TESTING TIME RANGES (Arabian Sea, 0-2000m, Temperature) ===')
for t in time_ranges:
    res = qs.get_insights_summary(region='Arabian Sea', time_range=t, depth='0–2000 m', variable='Temperature')
    cov = res['key_insights']['coverage']
    f_cnt = cov['active_floats']
    obs_cnt = cov['total_observations']
    print(f'Time [{t}]: Floats={f_cnt}, MatchingObs={obs_cnt:,}')

print('\n=== TESTING DEPTHS (Arabian Sea, Full Record, Temperature) ===')
for d in depths:
    ...
    obs_cnt = cov['total_observations']
    t_obs = tsig['observed']
    print(f'Depth [{d}]: Floats={f_cnt}, MatchingObs={obs_cnt:,}, TempObs={t_obs}')
